...
C++ extension, constructs the `ours_pro` hierarchy once, solves the Poisson and
biharmonic cube problems, and writes the combined figure plus a JSON summary
into `output/`.
...
def run_suite(resolution: int = 40, verbose: bool = False) -> dict[str, Any]:
    """Build the hierarchy once, solve both PDEs, and return the summary."""
    _ensure_python()
    _ensure_deps()
...
def main() -> int:
    ...
    json_path = output_dir / f"run_demo_cube{args.resolution}.json"
